#!/bin/ksh
# ============================================================
# MMI3G-Toolkit — Google Earth Deploy (v6 StreetView Test)
# Deploys GEMMI + v6 proxy with BearSSL TLS for StreetView
...
echo "============================================"
echo " Google Earth Deploy (v6 StreetView Test)"
echo "============================================"
echo ""
...
for f in gemmi_final libmessaging.so libthirdparty_icu_3_5.so mapStylesWrite ge_settings.dat gemmi_models_res.zip dbRoot_custom.bin auth_resp1.bin auth_resp2.bin gemmi_control.sh gemmi_server.sh gemmi_proxy gemmi_proxy_v6 run_gemmi.sh; do
    if [ -f "${GEMMI_SRC}/${f}" ]; then
...
# === V6 PROXY: Replace v5 with v6 (BearSSL TLS for StreetView) ===
if [ -f "${GEMMI_DST}/gemmi_proxy_v6" ]; then
    rm -f "${GEMMI_DST}/gemmi_proxy" 2>/dev/null
    mv "${GEMMI_DST}/gemmi_proxy_v6" "${GEMMI_DST}/gemmi_proxy"
    chmod +x "${GEMMI_DST}/gemmi_proxy"
    echo "[OK] V6 proxy installed (BearSSL TLS for StreetView)"
else
    echo "[WARN] gemmi_proxy_v6 not found, using v5"
fi
