...
implements ElementParserIFC {
    ...
    public ScreenElementModel parseElement(String[] stringArray, TokenReaderIFC tokenReaderIFC) throws ParseException {
        BIOSCtrlModel bIOSCtrlModel = null;
        if (!stringArray[0].toLowerCase().equals("biosctrl")) {
            throw new ParseException("Wrong parser used: BiosctrlParser!");
        }
        bIOSCtrlModel = new BIOSCtrlModel();
        try {
            while (tokenReaderIFC.ready()) {
                ...
                if (stringArray2[0].toLowerCase().equals("value")) {
                    ...
                    try {
                        if (stringArray2[1].equals("int")) {
                            bIOSCtrlModel.setValueInt(this.getDataSource(stringArray2[2]), Integer.decode(stringArray2[3]), Long.decode(stringArray2[4]));
                        }
                        if (!stringArray2[1].equals("String")) continue;
                        bIOSCtrlModel.setValueString(this.getDataSource(stringArray2[2]), Integer.decode(stringArray2[3]), Long.decode(stringArray2[4]));
                        continue;
                    }
                ...
                if (stringArray2[0].equals("label")) {
                    ...
                    bIOSCtrlModel.setLabel(stringArray2[1]);
                    continue;
                }
                if (stringArray2[0].equals("entry")) {
                    bIOSCtrlModel.addEntry(stringArray2[1], stringArray2[2]);
                    continue;
                }
                if (stringArray2[0].equals("poll")) {
                    ...
                    try {
                        bIOSCtrlModel.setRefreshInterval(Integer.decode(stringArray2[1]));
                        continue;
                    }
                    catch (NumberFormatException numberFormatException) {
                ...
                if (stringArray2[0].equals("format")) {
                    ...
                    try {
                        bIOSCtrlModel.setFormat(Double.parseDouble(stringArray2[1]), stringArray2[2]);
                        continue;
                    }
                    catch (NumberFormatException numberFormatException) {
        ...
        if (!bIOSCtrlModel.areRequiredFieldsSet()) {
            throw new ParseException("required BiosCtrl elements missing");
        }
        return bIOSCtrlModel;
    }
}
