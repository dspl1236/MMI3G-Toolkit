...
extends Widget {
    ...
    protected TextElement statusLabel;
    private Path p;

    public RunnerWidget(ScreenElementModel screenElementModel, ScreenRenderer screenRenderer) {
    ...
    public void render(Graphics graphics, Graphics2D graphics2D, Composite composite) {
        ...
        RunnerModel runnerModel = (RunnerModel)this.model;
        if (!this.isVisible()) {
            return;
        }
        Font font = this.rendererConfig.getFont();
        if (this.teLabel == null) {
            this.teLabel = this.resMan.newTextElement(this.screenId, font, runnerModel.getLabel(), false);
        } else {
            this.teLabel.setString(runnerModel.getLabel());
        }
        int n2 = this.teLabel.getWidth() + 2 * this.rendererConfig.getBUTTON_BOX_WADDER();
        int n3 = this.rendererConfig.getFONT_SIZE() + 1;
        if (this.statusLabel == null) {
            this.statusLabel = this.resMan.newTextElement(this.screenId, font, runnerModel.getStatus(), false);
        } else {
            this.statusLabel.setString(runnerModel.getStatus());
        }
        ...
        if (this.isSelected()) {
            this.statusLabel.setColor(this.rendererConfig.getCOLOR_WHITE());
        } else {
            this.statusLabel.setColor(this.rendererConfig.getCOLOR_LIGHT_GREEN());
        }
        this.statusLabel.setPosition(this.x + this.rendererConfig.getCURSOR_X() * 280 + this.rendererConfig.getBUTTON_BOX_WADDER(), this.y + n3);
        ...
        composite.addDrawable((Drawable)this.statusLabel, 0);
    }
