...
extends Widget {
    ...
    public JobWidget(ScreenElementModel screenElementModel, ScreenRenderer screenRenderer) {
    ...
    public void render(Graphics graphics, Graphics2D graphics2D, Composite composite) {
        ...
        JobModel jobModel = (JobModel)this.model;
        if (!this.isVisible()) {
            return;
        }
        Font font = this.rendererConfig.getFont();
        if (this.teLabel == null) {
            this.teLabel = this.resMan.newTextElement(this.screenId, font, jobModel.getLabel(), false);
        } else {
            this.teLabel.setString(jobModel.getLabel());
        }
