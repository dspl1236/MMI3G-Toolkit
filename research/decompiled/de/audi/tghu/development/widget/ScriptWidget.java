...
extends Widget {
    ...
    public ScriptWidget(ScreenElementModel screenElementModel, LinkListener linkListener, ScreenRenderer screenRenderer) {
            ...
            case 17: {
                if (ScriptOutputReader.getInstance() == null || !ScriptOutputReader.getInstance().isScriptRunning()) {
                    if (this.listener != null) {
                        this.listener.linkClicked(0);
                    }
                    this.model.performAction();
                } else if (this.listener != null) {
                    this.listener.linkClicked(3);
                }
                bl = true;
            }
        }
    ...
    public void render(Graphics graphics, Graphics2D graphics2D, Composite composite) {
        ...
        ScriptModel scriptModel = (ScriptModel)this.model;
        if (!this.isVisible()) {
            return;
        }
        Font font = this.rendererConfig.getFont();
        if (this.teLabel == null) {
            this.teLabel = this.resMan.newTextElement(this.screenId, font, scriptModel.getLabel(), false);
        } else {
            this.teLabel.setString(scriptModel.getLabel());
        }
